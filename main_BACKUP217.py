...
def create_task(task: TaskCreate):
    ...
    try:
        with get_db() as conn:
            ...
            cursor.execute(
                """
                INSERT INTO tasks (id, text, completed, task_owner, created_at)
                VALUES (%s, %s, %s, %s, NOW())
                RETURNING *
                """,
                (task.id, task.text, task.completed, task.task_owner)
            )
    ...
    try:
        with get_db() as conn:
            ...
            cursor.execute(
                """
                UPDATE tasks
                SET text = %s, completed = %s, task_owner = %s
                WHERE id = %s
                RETURNING *
                """,
                (task.text, task.completed, task.task_owner, task_id)
            )
